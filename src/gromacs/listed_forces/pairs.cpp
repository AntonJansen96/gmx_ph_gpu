                             ...
                             int*                global_atom_index)
{
    ...
    for (i = 0; (i < nbonds);)
    {
        ...
        {
            case F_LJ14:
                bFreeEnergy = (fr->efep != efepNO && fr->electrostaticPotential.empty()
        ...
        else
        {
            ...
            fscal = evaluate_single(r2, fr->pairsTable->scale, fr->pairsTable->data.data(),
                                    fr->pairsTable->stride, qq, c6, c12, &velec, &vvdw, &coulombValue);
            if (potential)
            {
                potential[ai] += epsfac * fr->fudgeQQ * md->chargeA[aj] * coulombValue;
                potential[aj] += epsfac * fr->fudgeQQ * md->chargeA[ai] * coulombValue;
            }
        }
...
void do_pairs(int                      ftype,
              ...
              int*                     global_atom_index)
{
    if (ftype == F_LJ14 && fr->ic->vdwtype != evdwUSER && !EEL_USER(fr->ic->eeltype) && !havePerturbedInteractions
        && (!stepWork.computeVirial && !stepWork.computeEnergy) && fr->electrostaticPotential.empty())
    {
