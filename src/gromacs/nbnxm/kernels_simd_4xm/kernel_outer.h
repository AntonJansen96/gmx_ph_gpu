...

{
    ...
    real * gmx_restrict pot    = out->potential.data();
#endif
    ...
    SimdReal mrc_3_S  = SimdReal(-2*ic->k_rf);
#if defined CALC_ENERGIES || GMX_COMPUTE_ELECTROSTATIC_POTENTIAL
    SimdReal hrc_3_S  = SimdReal(ic->k_rf);
    SimdReal moh_rc_S = SimdReal(-ic->c_rf);
#endif
#endif

#ifdef CALC_COUL_TAB

    SimdReal invtsp_S  = SimdReal(ic->tabq_scale);
#if defined CALC_ENERGIES || GMX_COMPUTE_ELECTROSTATIC_POTENTIAL
    SimdReal mhalfsp_S = SimdReal(-0.5/ic->tabq_scale);
#endif
...
#if (defined CALC_COUL_TAB || defined CALC_COUL_EWALD) && (defined CALC_ENERGIES || GMX_COMPUTE_ELECTROSTATIC_POTENTIAL)
    SimdReal sh_ewald_S = SimdReal(ic->sh_ewald);
#endif
...
#if GMX_COMPUTE_ELECTROSTATIC_POTENTIAL
                    pot[sci + ia] -= facel*qi*2*Vc_sub_self;
#endif
                }
            }

#if defined CALC_ENERGIES && defined LJ_EWALD_GEOM
            {
                int  ia;
...
#if GMX_COMPUTE_ELECTROSTATIC_POTENTIAL
        SimdReal pot_S0  = setZero();
        SimdReal pot_S1  = setZero();
        SimdReal pot_S2  = setZero();
        SimdReal pot_S3  = setZero();
#endif
...
#ifdef GMX_COMPUTE_ELECTROSTATIC_POTENTIAL
        SimdReal facel_S(facel);
        pot_S0 = facel_S*pot_S0;
        pot_S1 = facel_S*pot_S1;
        pot_S2 = facel_S*pot_S2;
        pot_S3 = facel_S*pot_S3;
        reduceIncr4ReturnSum(pot + sci, pot_S0, pot_S1, pot_S2, pot_S3);
#endif
...
#undef GMX_COMPUTE_ELECTROSTATIC_POTENTIAL
