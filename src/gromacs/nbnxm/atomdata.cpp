...
void nbnxn_atomdata_set_charges(nbnxn_atomdata_t     *nbat,
                                const Nbnxm::GridSet &gridSet,
                                const real           *charge)
{
 ...
 */
static void
nbnxn_atomdata_accumulate_real_part(const Nbnxm::GridSet          &gridSet,
                                    const real                    *data,
                                    const int                      a0,
                                    const int                      a1,
                                    real *                         buffer)
...
static void nbnxn_atomdata_add_nbat_f_to_f_stdreduce(nbnxn_atomdata_t *nbat,
                                                     int               nth)
{
    constexpr int stride = (reductionBuffer == ReductionBuffer::Force ? 3 : 1);
    if (reductionBuffer == ReductionBuffer::Force)
    {
        GMX_ASSERT(nbat->fstride == 3, "Hardcoded assumption of fstride=3");
    }
    ...
    for (int th = 0; th < nth; th++)
    {
        try
        {
            ...
            for (int b = b0; b < b1; b++)
            {
                ...
                {
                    nbnxn_atomdata_clear_reals(bufferRef<reductionBuffer>(&nbat->out[0]),
                                               i0, i1);
                }
            }
        }
...
static void getAtomStartAndNumber(const Nbnxm::AtomLocality  locality,
                                  const Nbnxm::GridSet      &gridSet,
                                  int                       *atomStart,
                                  int                       *numAtoms)
{
...
#pragma omp parallel for num_threads(nth) schedule(static)
    for (int th = 0; th < nth; th++)
    {
        try
        {
            nbnxn_atomdata_accumulate_real_part(gridSet,
                                                nbat->out[0].potential.data(),
                                                a0 + ((th + 0)*na)/nth,
                                                a0 + ((th + 1)*na)/nth,
                                                buffer);
        }
