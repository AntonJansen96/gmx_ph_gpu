...
void nbnxn_atomdata_t::resizeForceBuffers()
{
    ...
    for (nbnxn_atomdata_output_t& outBuffer : out)
    {
        ...
        {
            outBuffer.potential.resize(paddedSize);
        }
    }
}
...
void nbnxn_atomdata_set_charges(nbnxn_atomdata_t* nbat, const Nbnxm::GridSet& gridSet, ArrayRef<const real> charges)
{
...
/* Add part of a real from nbnxn_atomdata_t to buffer
 *
 * Note: Adding restrict to f makes this function 50% slower with gcc 7.3
 */
static void nbnxn_atomdata_accumulate_real_part(const Nbnxm::GridSet& gridSet,
                                                const real*           data,
                                                const int             a0,
                                                const int             a1,
...
enum class ReductionBuffer
{
    Force,
    ElectrostaticPotential
};

template<ReductionBuffer reductionBuffer>
static gmx::ArrayRef<real> bufferRef(nbnxn_atomdata_output_t* out)
{
    if (reductionBuffer == ReductionBuffer::Force)
    {
        return out->f;
    }
    else
...
{
    const int stride = (reductionBuffer == ReductionBuffer::Force ? nbat->fstride : 1);
...
#pragma omp parallel for num_threads(nth) schedule(static)
    for (int th = 0; th < nth; th++)
    {
        try
        {
            ...
            for (int b = b0; b < b1; b++)
            {
                int i0 = b * NBNXN_BUFFERFLAG_SIZE * stride;
                int i1 = (b + 1) * NBNXN_BUFFERFLAG_SIZE * stride;
                ...
                for (gmx::index out = 1; out < gmx::ssize(nbat->out); out++)
                {
                    ...
                    {
                        fptr[nfptr++] = bufferRef<reductionBuffer>(&nbat->out[out]).data();
                    }
                }
                if (nfptr > 0)
                {
                    ...
                    nbnxn_atomdata_reduce_reals
#endif
                            (bufferRef<reductionBuffer>(&nbat->out[0]).data(),
                             bitmask_is_set(flags[b], 0), fptr, nfptr, i0, i1);
                }
                else if (!bitmask_is_set(flags[b], 0))
                {
                    nbnxn_atomdata_clear_reals(bufferRef<reductionBuffer>(&nbat->out[0]), i0, i1);
                }
            }
        }
        GMX_CATCH_ALL_AND_EXIT_WITH_FATAL_ERROR
...
static void getAtomStartAndNumber(const gmx::AtomLocality locality,
                                  const Nbnxm::GridSet&   gridSet,
                                  int*                    atomStart,
                                  int*                    numAtoms)
{
    switch (locality)
    {
        case gmx::AtomLocality::All:
            *atomStart = 0;
            *numAtoms  = gridSet.numRealAtomsTotal();
            break;
        case gmx::AtomLocality::Local:
            *atomStart = 0;
            *numAtoms  = gridSet.numRealAtomsLocal();
            break;
        case gmx::AtomLocality::NonLocal:
...
void reduceForces(nbnxn_atomdata_t* nbat, const gmx::AtomLocality locality, const Nbnxm::GridSet& gridSet, rvec* f)
{
    int a0 = 0;
    int na = 0;
    getAtomStartAndNumber(locality, gridSet, &a0, &na);
    ...
    if (nbat->out.size() > 1)
    {
        ...
        {
            gmx_incons("reduceForces called with nout>1 and locality!=eatAll");
        }
        ...
        {
            nbnxn_atomdata_add_nbat_f_to_f_stdreduce<ReductionBuffer::Force>(nbat, nth);
        }
    }
#pragma omp parallel for num_threads(nth) schedule(static)
    for (int th = 0; th < nth; th++)
...
void reduceElectrostaticPotential(nbnxn_atomdata_t*       nbat,
                                  const gmx::AtomLocality locality,
                                  const Nbnxm::GridSet&   gridSet,
                                  gmx::ArrayRef<real>     buffer)
{
    ...
    if (nbat->out.size() > 1)
    {
        ...
        /* Reduce the thread output buffers into buffer 0, before adding
         * them to the, differently ordered, "real" buffer.
         */
        nbnxn_atomdata_add_nbat_f_to_f_stdreduce<ReductionBuffer::ElectrostaticPotential>(nbat, nth);
    }
#pragma omp parallel for num_threads(nth) schedule(static)
    for (int th = 0; th < nth; th++)
    {
        try
        {
            nbnxn_atomdata_accumulate_real_part(gridSet, nbat->out[0].potential.data(),
                                                a0 + ((th + 0) * na) / nth,
                                                a0 + ((th + 1) * na) / nth, buffer);
        }
        GMX_CATCH_ALL_AND_EXIT_WITH_FATAL_ERROR;
    }
}
