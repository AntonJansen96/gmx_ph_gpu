 ...
 */
 ...
 #define CALC_ELECTROSTATIC_POTENTIAL 1
 ...
 #ifdef CALC_ENERGIES
 #ifdef EL_EWALD_ANY
    //  float  beta        = nbparam.ewald_beta; // anton: temporary fix
    //  float  ewald_shift = nbparam.sh_ewald;
 #else
 ...
 #if defined CALC_ENERGIES || CALC_ELECTROSTATIC_POTENTIAL
 ...
 #if defined CALC_ENERGIES && defined LJ_EWALD
 ...
 #endif                                  /* CALC_ENERGIES || CALC_ELECTROSTATIC_POTENTIAL */
 ...
 #endif
 #if defined CALC_ENERGIES || CALC_ELECTROSTATIC_POTENTIAL
                                 float func_el = inv_r * (int_bit - erff(r2 * inv_r * beta)) - int_bit * ewald_shift;
 #endif
 #ifdef CALC_ENERGIES
 #ifdef EL_EWALD_ANY
                                 /* 1.0f - erff is faster than erfcf */
                                 E_el    += qi * qj_f * func_el;
 #endif                          /* EL_EWALD_ANY */
 #endif
                                 f_ij    = rv * F_invr;
 ...
 #if CALC_ELECTROSTATIC_POTENTIAL
 #ifdef EL_EWALD_ANY
                                 fcj_buf.w    += qi * func_el;
                                 fci_buf[i].w += nbparam.epsfac * qj_f * func_el;
 #endif
 #endif
                             }
                         }
 ...
 #undef CALC_ELECTROSTATIC_POTENTIAL
