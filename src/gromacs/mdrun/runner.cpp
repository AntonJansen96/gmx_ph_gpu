...
static bool gpuAccelerationOfNonbondedIsUseful(const MDLogger& mdlog, const t_inputrec& ir, bool issueWarning)
{
    ...
    if (ir.lambda_dynamics)
    {
        gpuIsUseful = false;
        warning =
                "Electrostatic potential calculation for constant pH is currently only im  plented "
                "in the CPU kernels, not on the GPU.";
    }
...
int Mdrunner::mdrunner()
{
    ...
    if (inputrec->lambda_dynamics)
    {
        if (DOMAINDECOMP(cr))
        {
            gmx_fatal(FARGS, "Lambda dynamics is not (yet) supported with domaind decomposition");
        }
        /* Initialization, constant ph */
        constantPH = std::make_unique<ConstantPH>(
                *inputrec, mtop.natoms, opt2fn("-cpHMD", filenames.size(), filenames.data()));
    }
    ...
    if (startingBehavior != StartingBehavior::NewSimulation)
    {
        ...
        applyLocalState(*inputHolder_.get(), logFileHandle, cr, domdecOptions.numCells,
                        inputrec.get(), globalState.get(), constantPH.get(), &observablesHistory,
