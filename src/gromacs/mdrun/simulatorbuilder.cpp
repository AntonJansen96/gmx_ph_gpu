...
std::unique_ptr<ISimulator> SimulatorBuilder::build(bool useModularSimulator)
{
    ...
    if (useModularSimulator)
    {
        ...
        return std::unique_ptr<ModularSimulator>(new ModularSimulator(
                std::make_unique<LegacySimulatorData>(
                        ...
                        centerOfMassPulling_->pull_work, ionSwapping_->ionSwap, topologyData_->top_global,
                        simulatorStateData_->globalState_p, simulatorStateData_->observablesHistory_p,
                        topologyData_->mdAtoms, profiling_->nrnb, profiling_->wallCycle,
                        legacyInput_->forceRec, simulatorStateData_->enerdata_p,
                        simulatorStateData_->ekindata_p, simulatorConfig_->runScheduleWork_,
                        *replicaExchangeParameters_, membedHolder_->membed(),
                        profiling_->walltimeAccounting, std::move(stopHandlerBuilder_),
                        simulatorConfig_->mdrunOptions_.rerun, constantph_->constantph),
    ...
    return std::unique_ptr<LegacySimulator>(new LegacySimulator(
            ...
            simulatorStateData_->ekindata_p, simulatorConfig_->runScheduleWork_, *replicaExchangeParameters_,
