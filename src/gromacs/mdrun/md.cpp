...
void gmx::LegacySimulator::do_md()
{
    ...
    while (!bLastStep)
    {
        ...
        else
        {
            ...
            if (constantph)
            {
                constantph->setLambdaCharges(mdatoms);
            }
        ...
        do_md_trajectory_writing(fplog, cr, nfile, fnm, step, step_rel, t, ir, state, state_global,
                                 constantph, observablesHistory, top_global, fr, outf, energyOutput,
                                 ekind, f.view().force(), checkpointHandler->isCheckpointingStep(),
        ...
        if (constantph)
        {
            /* Update lambda with the charge interpolation method */
            state->lambda_therm_integral -= constantph->updateLambdas(*ir, t, step);
        }
