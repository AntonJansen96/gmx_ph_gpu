              ...
              const DDBalanceRegionHandler&       ddBalanceRegionHandler)
{
    ...
    if (fr->constantPH)
    {
        // We copy before the non-local atom setup (also PME ranks don't work)
        GMX_RELEASE_ASSERT(!DOMAINDECOMP(cr), "DD not implemented yet for constant pH");

        // We need to update the charges in the Nbnxm module
        fr->nbv->setAtomCharges(*mdatoms);

        // Clear the potential buffer elements that will be used
        // TODO: Move this into force outputs
        for (int i = 0; i < mdatoms->nr; i++)
        {
            fr->electrostaticPotential[i] = 0;
        }
    }
    ...
    if (stepWork.computeNonbondedForces && !useOrEmulateGpuNb)
    {
        ...
        if (stepWork.computeForces)
        {
            ...
            if (fr->constantPH)
            {
                nbv->addElectrostaticPotential(AtomLocality::All, fr->electrostaticPotential, wcycle);
            }
        }
