...
void pr_inputrec(FILE* fp, int indent, const char* title, const t_inputrec* ir, gmx_bool bMDPformat)
{
    ...
    if (available(fp, ir, indent, title))
    {
        ...
        PS("lambda tcoupl", ELAMBDATCOUPLTYPE(ir->eLambdaThermostat));
        PS("lambda charge constraints", EBOOL(ir->bLambdaChargeConstraints));
        PS("lambda multi state constraints", EBOOL(ir->bLambdaMultiStateConstraints));
        PS("lambda dynamics calibration run", EBOOL(ir->bLambdaIsCalibration));
