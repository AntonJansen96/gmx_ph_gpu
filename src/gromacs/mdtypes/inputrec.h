...
struct t_inputrec // NOLINT (clang-analyzer-optin.performance.Padding)
{
    ...
    bool lambda_dynamics;
    //! Thermostat for lambda dynamics.
    int eLambdaThermostat;
    //! If we are using charge constraint for lambda dynamics.
    bool bLambdaChargeConstraints;
    //! If there are multi state constraints for lambda dynamics.
    bool bLambdaMultiStateConstraints;
    //! If this lambda dynamics run is meant to be done for calibrating the reference dvdl values.
    bool bLambdaIsCalibration;
