...
enum cptv
{
    ...
    cptv_ConstantPH,       /**< Added checkpointing for constant pH code */
...
static const char* eConstantpHNames[eConstantpHNr] = { "constantph_number", "constantpH_data" };
...
static void do_cpt_header(XDR* xd, gmx_bool bRead, FILE* list, CheckpointHeaderContents* contents)
{
    ...
    if (contents->file_version >= cptv_ConstantPH)
    {
        do_cpt_int_err(xd, "constant ph flags", &contents->flagsConstantpH, list);
    }
    else
    {
...
static int doCptConstantpH(XDR* xd, bool bRead, int fflags, ConstantPH* constantpH, FILE* list)
{
    int                           ret         = 0;
    int                           numResidues = 0;
    std::vector<LambdaResidues>   tmpResidues;
    gmx::ArrayRef<LambdaResidues> lambdaResidues;
    if (constantpH != nullptr)
    {
        GMX_RELEASE_ASSERT(fflags != 0, "Can't have constant pH and no flags set.");
        lambdaResidues = constantpH->lambdaResidueHandle()->residues_;
        numResidues    = lambdaResidues.size();
    }
    for (int i = 0; (i < eConstantpHNr && ret == 0); i++)
    {
        if (fflags & (1 << i))
        {
            switch (i)
            {
                // no checking so far that we have the same number of things,
                // we just read the stuff raw from disk.
                case eConstantpH_NUMBER:
                    do_cpt_int_err(xd, eConstantpHNames[i], &numResidues, list);
                    if (bRead && (constantpH == nullptr))
                    {
                        tmpResidues.resize(numResidues);
                        lambdaResidues = tmpResidues;
                    }
                    break;
                case eConstantpH_DATA:
                    for (auto& lambdaResidue : lambdaResidues)
                    {
                        do_cpt_real_err(xd, &lambdaResidue.lambda.x0);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.x);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.x_old);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.x_old_old);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.v);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.v_old);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.v_old_old);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.ekin);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.T);
                        do_cpt_int_err(xd, eConstantpHNames[i], &lambdaResidue.lambda.tau, list);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.bar);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.dvdl);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.dvdl_old);

                        do_cpt_real_err(xd, &lambdaResidue.lambda.dvdl_pot);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.dvdl_ph);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.dvdl_dwp);
                        do_cpt_real_err(xd, &lambdaResidue.lambda.dvdl_ref);
                    }
                    break;
            }
        }
                           ...
                           gmx::WriteCheckpointDataHolder*   modularSimulatorCheckpointData)
{
    ...
    if ((do_cpt_state(gmx_fio_getxdr(fp), state->flags, state, nullptr) < 0)
        ...
        || (do_cpt_swapstate(gmx_fio_getxdr(fp), FALSE, headerContents.eSwapCoords,
            ...
            < 0)
        || (doCptConstantpH(gmx_fio_getxdr(fp), FALSE, headerContents.flagsConstantpH, constantpH, nullptr) < 0)
                            ...
                            bool                           useModularSimulator)
{
    ...
    *init_fep_state = state->fep_state; /* there should be a better way to do this than setting it
    ...
    if (headerContents->flagsConstantpH)
    {
        if (constantpH == nullptr)
        {
            gmx_fatal(FARGS, "Can't continue cpHMD simulation without cpHMD being initialized");
        }
        ret = doCptConstantpH(gmx_fio_getxdr(fp), TRUE, headerContents->flagsConstantpH, constantpH, nullptr);
        if (ret)
        {
            cp_error();
        }
                     ...
                     bool                           useModularSimulator)
{
    ...
    if (SIMMASTER(cr))
    {
        ...
        read_checkpoint(fn, logfio, cr, dd_nc, ir->eI, &(ir->fepvals->init_fep_state), &headerContents,
                        state, constantpH, observablesHistory, reproducibilityRequested,
                                                     ...
                                                     std::vector<gmx_file_position_t>* outputfiles)
{
    ...
    ret = doCptConstantpH(gmx_fio_getxdr(fp), TRUE, headerContents.flagsConstantpH, nullptr, nullptr);
...
void list_checkpoint(const char* fn, FILE* out, ConstantPH* constantpH)
{
    t_fileio* fp;
    int       ret;
    ...
    if (ret == 0)
    {
        ret = doCptConstantpH(gmx_fio_getxdr(fp), TRUE, headerContents.flagsConstantpH, constantpH, out);
    }
