 ...
 */
static const char *tpx_tag = "constant-ph";
                        ...
                        int file_version)
{
    ...
    if (strcmp(file_tag, tpx_tag) == 0)
    {
        ...
        gmx_fio_do_gmx_bool(fio, ir->lambda_dynamics);
    }
...
static void do_tpxheader(t_fileio *fio, gmx_bool bRead, t_tpxheader *tpx,
                         gmx_bool TopOnlyOK,
                         char *file_tag, int *fileVersionPointer, int *fileGenerationPointer)
{
    char      buf[STRLEN];
    gmx_bool  bDouble;
    int       precision;
                  ...
                  gmx_mtop_t *mtop)
{
    ...
    do_tpxheader(fio, bRead, &tpx, TopOnlyOK,
                 fileTag, &fileVersion, &fileGeneration);
...
#define do_test(fio, b, p) if (bRead && ((p) != NULL) && !(b)) gmx_fatal(FARGS, "No %s in %s",#p, gmx_fio_getname(fio))
    ...
    if (tpx.bIr)
    {
        ...
        {
            do_inputrec(fio, ir, bRead, fileTag, fileVersion);
            if (fileVersion < 51)
            {
...
void read_tpxheader(const char *fn, t_tpxheader *tpx, gmx_bool TopOnlyOK)
{
    ...
    fio = open_tpx(fn, "r");
    do_tpxheader(fio, TRUE, tpx, TopOnlyOK, fileTag, nullptr, nullptr);
    close_tpx(fio);
}
