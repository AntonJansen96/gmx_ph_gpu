...
void gmx_pme_calc_energy(struct gmx_pme_t *pme, int n, rvec *x, real *q, real *V)
{
    ...
    pme_atomcomm_t &atc = pme->atc_energy;
    atc.nthread         = 1;
...
int gmx_pme_do(struct gmx_pme_t *pme,
               ...
               int flags,
               real pot[], gmx::ArrayRef<const int> potentialAtoms)
{
...
#pragma omp parallel for num_threads(pme->nthread) schedule(static)
        ...
        if (pme->nthread == 1)
        {
            ...
            {
                pot[a] += gather_potential_bsplines(*pme, pmegrid->grid.grid, pme->atc[0], pme->atc[0].spline[0], a);
...
#pragma omp parallel num_threads(pme->nthread) private(thread)
            {
                try
                {
                    ...
                    {
                        GMX_ASSERT(splineIndex == 0 || spline.ind[splineIndex - 1] < spline.ind[splineIndex], "spline.ind should be increasing");
                        ...
                        {
                            pot[a] += gather_potential_bsplines(*pme, pmegrid->grid.grid, pme->atc[0], spline, splineIndex);
                            i++;
                            splineIndex++;
                        }
                    }
