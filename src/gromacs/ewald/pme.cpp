...
void gmx_pme_calc_energy(gmx_pme_t* pme, gmx::ArrayRef<const gmx::RVec> x, gmx::ArrayRef<const real> q, real* V)
{
    ...
    const pmegrids_t& grid = pme->pmegrid[PME_GRID_QA];

    /* Only calculate the spline coefficients, don't actually spread */
    spread_on_grid(pme, atc, &grid, TRUE, FALSE, pme->fftgrid[PME_GRID_QA], FALSE, PME_GRID_QA);
...
#pragma omp parallel for num_threads(pme->nthread) schedule(static)
        ...
        // TODO: modified for constant pH
        // add a if condition that only call this if constant pH is on
        // grid_index = 0 means that if free_energy YES then only calculate potential for chargeA

        // also, modified pme-only.cpp to add gmx_pme_do new attributes

        if (pme->nthread == 1)
        {
            /* One thread, simple: loop over potentialAtoms */
            for (int a : potentialAtoms)
            {
                pot[a] += gather_potential_bsplines(*pme, pmegrid->grid.grid, pme->atc[0],
                                                    pme->atc[0].spline[0], a);
            }
        }
        else
        {
            /* Multiple threads: we need to find which potentialAtoms are on which thread */
#pragma omp parallel num_threads(pme->nthread) private(thread)
            {
                try
                {
                    const int           thread = gmx_omp_get_thread_num();
                    const splinedata_t& spline = pme->atc[0].spline[thread];

                    int i           = 0;
                    int splineIndex = 0;
                    while (i < potentialAtoms.ssize() && splineIndex < spline.n)
                    {
                        GMX_ASSERT(splineIndex == 0 || spline.ind[splineIndex - 1] < spline.ind[splineIndex],
                                   "spline.ind should be increasing");

                        const int a = potentialAtoms[i];
                        if (spline.ind[splineIndex] < a)
                        {
                            splineIndex++;
                        }
                        else if (spline.ind[splineIndex] > a)
                        {
                            i++;
                        }
                        else
                        {
                            pot[a] += gather_potential_bsplines(*pme, pmegrid->grid.grid,
                                                                pme->atc[0], spline, splineIndex);
                            i++;
                            splineIndex++;
                        }
                    }
