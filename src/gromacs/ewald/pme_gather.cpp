                               ...
                               const int             splineIndex)
{
    ...
    const int  *idxptr = atc.idx[spline.ind[splineIndex]];
    const int   norder = splineIndex*order;
...
real gather_energy_bsplines(const gmx_pme_t       &pme,
                            const real            *grid,
                            const pme_atomcomm_t  &atc)
{
    ...
    real                energy = 0;
    for (int i = 0; i < atc.n; i++)
    {
        const real coefficient = atc.coefficient[i];

        if (coefficient != 0)
        {
            real pot  = gather_potential_bsplines(pme, grid, atc, spline, i);

            energy   += coefficient*pot;
        }
    }
